...
/*
 * representa una obra del catalogo (no un ejemplar fisico individual eso lo maneja ejemplar.h) la unicidad del catalogo la da nombre
 */
typedef struct {
    char *nombre;
    char *autor;
...
/*
 * inserta en el catalogo los libros del lote que sean nuevos (por nombre) y con informacion valida, genera los ejemplares correspondientes, y muestra el reporte de los registros no procesados
 * Entradas: catalogo, cantidad de catalogo, ejemplares y cantidad de ejemplares, todos por referencia
 * (se modifican inplace), arreglo de lote y su cantidad
 * Salidas: ninguna (efecto: catalogo y ejemplares actualizados, reporte impreso)
 * Restricciones: valida unicidad por nombre y que los campos numericos/textuales sean correctos; los registros invalidos o duplicados se reportan y no se insertan
 * Objetivo: cumplir la funcionalidad incluir en lote
 */
void procesar_lote(Libro **catalogo, int *cantidad_catalogo, Ejemplar **ejemplares, int *cantidad_ejemplares, const Libro *lote, int cantidad_lote);
